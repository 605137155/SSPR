...
class miniDukeMTMC(Dataset):
    url = 'https://drive.google.com/uc?id=0B0VOCNYh8HeRdnBPa2ZWaVBYSVk'
    md5 = '2f93496f9b516d1ee5ef51c1d5e7d601'

    def __init__(self, root, split_id=0, num_val=1, download=True):
        super(miniDukeMTMC, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        fpath = osp.join(raw_dir, 'miniDukeMTMC-reID.zip')
        if not osp.isfile(fpath):
            raise RuntimeError("Please download the dataset manually from {} "
                               "to {}".format(self.url, fpath))

        # Extract the file
        exdir = osp.join(raw_dir, 'miniDukeMTMC-reID')
