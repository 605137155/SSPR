...
class IntraCameraTrainer(BaseTrainer):
    ...
    def train(self,
              cluster_epoch, #用于打印表示当前聚类epoch的参数
              epoch,         #用于打印的第一阶段的训练epoch
              data_loader,   #用于每个摄像头的data_loader,data_loader[i]表示第i个摄像头的data_loader
              optimizer,     #torch.optim.SGD(param_groups, momentum=0.9)
              print_freq=1,  #打印频率
              ):
        self.model.train()   #作用是启用batch normalization和drop out
        ...
        data_loader_size = min([len(l) for l in data_loader])  #用于控制打印的
        for i, inputs in enumerate(zip(*data_loader)):  #每个循环从每个摄像头data_loader数据抽取一个（第二个循环抽取第二个），并且打包成 元组，及每次训练的数据为从所有摄像头中抽取一个
            data_time.update(time.time() - end)
            for domain, domain_input in enumerate(inputs):#对每个数据
...
class InterCameraTrainer(BaseTrainer):
    ...
    def _forward(self, inputs, targets):
        ...
        if isinstance(self.criterion, torch.nn.CrossEntropyLoss): #如果是交叉熵损失
    ...
    def train(self,
        ...
        for i, inputs in enumerate(data_loader):
            ...
            loss_entropy, prec_entropy, loss_triple, prec_triple = self._forward(imgs, targets) #输出四类
            ...
            a = losses_entropy.avg
            b = precisions_entropy.avg
            c = losses_triple.avg
            d = precisions_triple.avg
        return a,b,c,d